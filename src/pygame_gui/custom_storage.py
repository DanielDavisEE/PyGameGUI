class MyList(dict):
    ...
    def __init__(self, node_name, node):
        super().__init__()
        self[node_name] = BlockObject(None, None, None, None, 0)
        self[node_name].surface = node
        self.node_name = node_name
        self.queue = []
    
    def append(self, name, parent, dimensions, coordinates, colour, priority=0):
        """name -> name of block
           parent -> name of parent block
           position -> euclidean coordinates of block
        """
        self[parent].children.append((name, priority))
        self[name] = BlockObject(parent, dimensions, coordinates, colour, priority)
        return None
...
class BlockObject():
    def __init__(self, parent, dimensions, coordinates, colour, priority):
        self.parent = parent
    ...
    def __repr__(self):
        return f'Block({self.parent}, {self.dimensions}, {self.coordinates}, {self.colour}, {self.children}, {self.priority}, {self.surface})'
